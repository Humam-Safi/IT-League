...
const Scoreboard: React.FC = () => {
  ...
  const groups = useMemo(() => {
    ...
    // Calculate dynamic stats for each team based on matchesTable
    ...
    return Object.keys(grouped)
      ...
      .map((groupName) => {
        const teams = grouped[groupName].sort((a, b) => {
          ...
          // 2. Direct Matches (Head-to-head)
          const h2hMatches = matchesData.filter(
            (m) =>
          ...
          h2hMatches.forEach((m) => {
            const isAA = m.teamA === a.name;
            const scoreA = isAA ? m.scoreA : m.scoreB;
            const scoreB = isAA ? m.scoreB : m.scoreA;
...
const GroupTable: React.FC<{ group: Group; index: number }> = ({
  group,
  index,
}) => {
  return (
      ...
      <div className="bg-slate-800/40 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col h-full hover:shadow-blue-500/10 transition-shadow duration-300 ring-1 ring-white/5">
        ...
        <div className="bg-gradient-to-r from-blue-600/30 to-slate-900/50 p-6 flex items-center justify-between border-b border-white/5 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-bold text-white flex items-center gap-3 relative z-10">
            ...
            <span className="tracking-tight text-blue-50">
              Group {group.name}
            </span>
          </h2>
        ...
        <div className="grid grid-cols-12 gap-2 p-4 text-xs font-bold text-blue-200/60 uppercase tracking-widest border-b border-white/5 bg-slate-900/40 text-center">
          ...
          <div className="col-span-1" title="Played">
            P
          </div>
          <div className="col-span-1" title="Goals Scored">
            +
          </div>
          <div className="col-span-1" title="Goals Received">
            -
          </div>
          <div className="col-span-2" title="Goal Difference">
            =
          </div>
          <div className="col-span-2" title="Points">
            Pts
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="col-span-4 flex items-center gap-3 pl-2 text-left">
                      ...
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src =
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(team.name)}&background=random`;
                        }}
                      />
                    </Link>
                  </div>
                  <Link
                    to={`/team/${team.id}`}
                    className="font-semibold text-sm md:text-base text-slate-100 truncate tracking-wide hover:text-blue-400 transition-colors"
                  >
                    {team.name}
                  </Link>
                </div>
                ...
                <div className="col-span-1 font-medium text-slate-300 flex items-center justify-center gap-1">
                  <span className="hidden group-hover:inline opacity-50 text-[10px]">
                    <Swords size={10} />
                  </span>
                  {team.goalsScored}
                </div>
