...
const getTeam = (teamName: string) => {
  return teamsData.find(t => t.name === teamName) || teamsData.find(t => t.id === teamName);
}

const Matches: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'results' | 'fixtures'>('fixtures');
        ...
        {activeTab === 'results' ? (
          <>
            ...
            {matchesData.length === 0 && (
              <div className="text-center text-slate-500 py-10">No matches found.</div>
            )}
          </>
        ) : (
           <>
            ...
            {nextMatchesData.length === 0 && (
              <div className="text-center text-slate-500 py-10">No upcoming matches.</div>
            )}
          </>
        )}
      </div>
  ...
  return (
    ...
    >
       ...
       <div className="bg-slate-900/50 px-4 py-2 flex items-center justify-center gap-2 text-xs text-blue-300 font-mono tracking-wider uppercase border-b border-white/5">
        <Calendar size={14} />
        {match.date} <span className="text-slate-600">|</span> <Clock size={14} /> {match.time}
      </div>
       ...
       <div className="p-4 md:p-6">
        <div className="flex flex-row items-center justify-between gap-1 md:gap-12">
          ...
          <div className="flex flex-col items-center flex-1 order-1">
            ...
            />
            <h3 className="text-sm md:text-2xl font-bold text-white text-center leading-tight">{match.teamA}</h3>
          </div>
          ...
          <div className="flex flex-col items-center flex-1 order-3">
             <img
              ...
              className="w-12 h-12 md:w-24 md:h-24 object-contain drop-shadow-lg mb-2"
            />
            <h3 className="text-sm md:text-2xl font-bold text-white text-center leading-tight">{match.teamB}</h3>
          </div>
        </div>
       </div>
    </motion.div>
      ...
      <div className="p-3 md:p-6">
        <div className="flex flex-row items-start justify-between gap-1 md:gap-12">
          ...
          <div className="flex flex-col items-center flex-1 order-1">
            ...
            />
            <h3 className="text-xs md:text-2xl font-bold text-white text-center leading-tight">{match.teamA}</h3>
            
            {/* Scorers A */}
            <div className="mt-2 md:mt-4 space-y-1 text-center">
              {match.scorersA?.map((scorer: string, i: number) => (
                <div key={i} className="text-[10px] md:text-sm text-emerald-400 font-medium flex items-center gap-1 justify-center">
                   ⚽ {scorer}
                </div>
              ))}
              {/* Cards A */}
              {match.cardsA?.map((card: any, i: number) => (
                <div key={i} className={`text-[10px] md:text-xs font-medium flex items-center gap-1 justify-center ${card.type === 'red' ? 'text-red-500' : 'text-yellow-500'}`}>
                   <div className={`w-3 h-4 rounded-sm ${card.type === 'red' ? 'bg-red-500' : 'bg-yellow-400'}`}></div>
                   {card.player}
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-col items-center flex-1 order-3">
             <img 
              ...
              className="w-10 h-10 md:w-24 md:h-24 object-contain drop-shadow-lg mb-2"
            />
            <h3 className="text-xs md:text-2xl font-bold text-white text-center leading-tight">{match.teamB}</h3>
             ...
             <div className="mt-2 md:mt-4 space-y-1 text-center">
              {match.scorersB?.map((scorer: string, i: number) => (
               ...
               {/* Cards B */}
              {match.cardsB?.map((card: any, i: number) => (
                <div key={i} className={`text-[10px] md:text-xs font-medium flex items-center gap-1 justify-center ${card.type === 'red' ? 'text-red-500' : 'text-yellow-500'}`}>
                   <div className={`w-3 h-4 rounded-sm ${card.type === 'red' ? 'bg-red-500' : 'bg-yellow-400'}`}></div>
                   {card.player}
                </div>
